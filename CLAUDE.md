...
- 어드민(`isAdmin` 가드)은 `/admin`(회원 관리: `AdminView.vue`)과 `/admin/live`(실시간 접속자: `AdminLiveView.vue`, `getActiveViewers`를 3초 폴링해 IP/회원/User-Agent/보는 글 표시) 두 탭으로 구성. 두 화면 상단에 탭 내비를 인라인으로 공유한다.
