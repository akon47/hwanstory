    ...
    stages {
        stage('Clone') {
            steps {
                ...
                git url: 'git@github.com:akon47/hwanstory.git', branch: 'master', credentialsId: GITHUB_CREDENTIALS_ID
            }
            post {
                success {
        ...
        stage('Docker Run') {
            steps {
                ...
                sshagent(credentials: ['ssh']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no kimhwan@10.10.10.100 'docker run -d --restart always --name ${APP_NAME} --net=host ${IMAGE_NAME}'"
